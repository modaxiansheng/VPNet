...
def compute_statistics_jit(overlaps,
    ...
    delta_idx = 0
    for i in range(gt_size):   #! 不要匹配完全不相关的gt框
        if ignored_gt[i] == -1:
            continue
...
def get_mAP(prec):
    ...
    for i in range(0, prec.shape[-1], 4):
        sums = sums + prec[..., i]   # prec.shape (3, 3, 2, 41)
    return sums / 11 * 100      # shape  3,3,2
...
def do_eval(gt_annos,
    ...
    # 计算3d的代码
    # ret = eval_class(dt_annos, dt_annos, current_classes, difficultys, 2, min_overlaps)
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    min_overlaps = min_overlaps[:, :, current_classes]  #shape (2,3,3)
    result = ''
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]): # minoverlaps.shape (2, 3, 3)
            result += print_str(
