...
class PointNet2MSG_gcn(nn.Module):
    def __init__(self, model_cfg, input_channels, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        ...
        # ________GCN模块
        # self.gcn3 = PlainGCNcopy(model_cfg,NUM_FILTERS=[64,64,64],input_channels=64)
        self.gcn = PlainGCNcopy(model_cfg,NUM_FILTERS=[128],input_channels=128)
        # ________
    ...
    def forward(self, batch_dict):
        ...
        # point_features1 = l_features[4].permute(0, 2, 1).contiguous()  # (B, N, C)
        # dict1 = {'point_features':point_features1.view(-1, point_features1.shape[-1]),
        #         'point_coords':torch.cat((batch_idx[:, None].float(), l_xyz[4].view(-1, 3)), dim=1),
        #         }
        # l_features[4] = (self.gcn(dict1))['point_features']
        ...
        point_features = l_features[0].permute(0, 2, 1).contiguous()  # (B, N, C) #shape:torch.Size([2, 16384, 128])


        batch_dict['point_features'] = point_features.view(-1, point_features.shape[-1])  #torch.Size([32768, 128])
        
        batch_dict['point_coords'] = torch.cat((batch_idx[:, None].float(), l_xyz[0].view(-1, 3)), dim=1)

        batch_dict = self.gcn(batch_dict)
