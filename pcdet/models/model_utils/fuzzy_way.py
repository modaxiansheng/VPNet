...
class fuzzy_sim():
    def __init__(self):
        #设置各个变量范围：
        self.in_volume_range = np.arange(0, 31, 1, np.float32)#体积取值（0，25）
        ...
        self.in_volume['N'] = fuzz.trimf(self.in_volume_range, [0, 0, 6])#行人与自行车体积
        self.in_volume['P'] = fuzz.trimf(self.in_volume_range, [4, 12, 30])#汽车体积

        self.in_density['N'] = fuzz.trimf(self.in_density_range, [0, 0, 0.1])#小密度
        self.in_density['M'] = fuzz.trimf(self.in_density_range, [0.1, 0.2, 0.7])#中等密度
        self.in_density['P'] = fuzz.trimf(self.out_thresh_range, [0.5, 1.0, 1.0])#大密度
